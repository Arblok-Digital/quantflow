...
| Test | `npx vitest run` ? **76/76 PASS** (8 files) � `npx tsc --noEmit` ? EXIT 0 |
...
```
server.ts                    ? Thin bootstrap & router registration (119 baris)
src/server/routes/           ? auth.ts, broker.ts, market.ts, ledger.ts, ai.ts, replay.ts, wsProxy.ts, _utils.ts
paperBook.ts                 ? PAPER BOOK BARREL (re-export dari src/paperbook/*) — logika engine/fill/bracket/mark sudah dipecah ke src/paperbook/{store,fill,bracketMonitor,markCache,config,types,errors}.ts
...
| Broker | `GET /api/broker/status` � `GET /api/broker/ticker` � `GET /api/broker/orderbook` � `GET /api/broker/klines` � `GET /api/broker/balance` � `POST /api/broker/credentials` � `POST /api/broker/credentials/clear` � `GET /api/broker/credentials/status` � `POST /api/broker/test` � `GET /api/broker/guardrails` � `POST /api/broker/kill` � `POST /api/broker/arm` � `POST /api/broker/disarm` � `POST /api/broker/order` � `GET /api/broker/positions` � `POST /api/broker/close` � `POST /api/broker/cancel` � `GET /api/broker/orders` � `POST /api/broker/position/update` � `GET /api/broker/order-status/:orderId` � `GET /api/broker/events` |
| Market | `GET /api/market/stream` (SSE, public) � `GET /api/market-feed` (REST) � `GET /api/klines` � `GET /api/onchain/bitcoin` � `GET /api/market/keel-context` � `GET /api/pump-scan` |
...
- Split `server.ts` monolith (2,227L) ke `src/server/routes/*` (auth, broker, market, ledger, ai, replay, wsProxy, _utils).
...
4. **Split `server.ts`** � SELESAI (`server.ts` bootstrap + route modules di `src/server/routes/*`).
