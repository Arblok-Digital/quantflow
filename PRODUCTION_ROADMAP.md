...
- [ ] **2.1** Bind default `127.0.0.1` (env `HOST` untuk override) + `helmet` + `cors` allowlist
...
- [ ] **2.3** **Server-side risk enforcement** di `placeBrokerOrder`: kill-switch flag, max daily loss, max open positions, max margin per posisi, cooldown antar order — order ditolak server dengan alasan eksplisit
...
- [ ] **2.5** Rate limit (express-rate-limit) di semua /api
- [ ] **2.6** Vault credential: enkripsi at-rest beneran (DPAPI via machine key / AES dengan key turunan passphrase), catatan: `mode: 0o600` di Windows TIDAK berfungsi; tambah `GET /api/broker/credentials/status` (mask key, tampilkan 4 char terakhir + exchange + testnet)
...
- [ ] **2.8** Live-mode double-lock: order live hanya jalan jika `TRADING_MODE=live` **DAN** vault punya flag `liveArmed: true` yang di-set lewat UI dengan konfirmasi ketik manual (anti one-click money loss)
